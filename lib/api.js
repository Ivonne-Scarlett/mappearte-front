...
export async function getArtByUserId() {
  const response = await fetch(`${API_URL}/userart`);
  return await response.json();
}

export async function registerPhoto( data) {
  const response = await fetch(`${API_URL}/streetart`, {
    method: "POST",
    body: JSON.stringify(data),
    headers: {
  ...
  const responseRegisterPhoto = await response.json();
  return responseRegisterPhoto;
}